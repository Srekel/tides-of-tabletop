...
pub fn main() !void {
    ...
    while (!rl.windowShouldClose()) {
        ...
        if (state.creating_character == .inactive) {
            ...
            if (rg.guiButton(rl.Rectangle.init(5, 5, 32, 32), rg.guiIconText(@intFromEnum(rg.GuiIconName.icon_file_new), "")) != 0) {
                if (state.creating_character == .inactive) {
            ...
            if (rg.guiButton(rl.Rectangle.init(40, 5, 32, 32), rg.guiIconText(@intFromEnum(rg.GuiIconName.icon_file_open), "")) != 0) {
        ...
        if (state.creating_character == .naming) {
            ...
            const name_choice = rg.guiTextBox(rl.Rectangle.init(350, 215, 200, 30), @ptrCast(&state.buf), 20, state.text_focus == .char_name);
            if (name_choice == 1) {
                state.text_focus = if (state.text_focus == .char_name) .none else .char_name;
            }
            label("Player name:", 220, 260, 255, 255, 255);
            const player_choice = rg.guiTextBox(rl.Rectangle.init(350, 255, 200, 30), @ptrCast(&state.buf2), 20, state.text_focus == .player_name);
            if (player_choice == 1) {
                state.text_focus = if (state.text_focus == .player_name) .none else .player_name;
            }

            if (rg.guiButton(rl.Rectangle.init(550, 215, 100, 30), rg.guiIconText(@intFromEnum(rg.GuiIconName.icon_help), "Random!")) != 0) {
        ...
        if (state.player_list_visible) {
            ...
            if (state.player_active >= 0 and rg.guiButton(rl.Rectangle.init(100, 240, 70, 30), rg.guiIconText(@intFromEnum(rg.GuiIconName.icon_file_open), "Load")) != 0) {
    ...
    for (player.expertises.slice(), 0..) |*expertise, i_e| {
        ...
        if (state.creating_character == .inactive and (expertise.level != .Master or expertise.points != 4)) {
            ...
            if (rg.guiButton(
                ...
                rg.guiIconText(@intFromEnum(rg.GuiIconName.icon_arrow_up_fill), ""),
            ) != 0) {
